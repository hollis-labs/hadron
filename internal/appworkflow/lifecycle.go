...
func NewWorkflowLifecycleService(options WorkflowLifecycleOptions) (*WorkflowLifecycleService, error) {
	...
	return &WorkflowLifecycleService{identity: options.Identity, contracts: options.Contracts, authoring: options.Authoring, exposure: options.Exposure}, nil
}
...
func (s *WorkflowLifecycleService) RegisterWorkflowDraft(ctx context.Context, request RegisterWorkflowDraftRequest) (WorkflowRegistrationResult, error) {
	...
	result, authorErr := s.authoring.author(ctx, agentRequest(request.Draft, &request.Suite, request.MakeCurrent), binding.Principal, true)
