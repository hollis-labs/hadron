        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', maxWidth: '640px' }}>
          <div className="hud-panel" style={{ padding: '1rem' }}>
            ...
            <div style={{ fontSize: '0.85rem', marginBottom: '0.3rem' }}>
              <strong style={{ color: 'rgb(var(--ok))' }}>HADRON</strong>
              <span style={{ color: 'rgb(var(--muted))', marginLeft: '0.5rem' }}>by Hollis Labs</span>
            </div>
          ...
          <div className="hud-panel" style={{ padding: '1rem' }}>
            ...
            <ol style={{ fontSize: '0.8rem', lineHeight: '1.7', paddingLeft: '1.2rem', color: 'rgb(var(--text))' }}>
                ...
                {[
                  ...
                  <tr key={key}>
                    <td style={{ padding: '0.3rem 0', width: '120px' }}>
                      <kbd style={{
                        background: 'rgb(var(--panel2))', border: '1px solid rgb(var(--border))',
                        borderRadius: '3px', padding: '2px 8px', fontSize: '0.75rem',
                        fontFamily: 'var(--font-mono)',
                      }}>{key}</kbd>
                    </td>
                    <td style={{ padding: '0.3rem 0', fontSize: '0.8rem', color: 'rgb(var(--muted))' }}>{desc}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="hud-panel" style={{ padding: '1rem' }}>
            ...
            <div style={{ fontSize: '0.78rem', color: 'rgb(var(--muted))', lineHeight: '1.6' }}>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>Dashboard</strong> — Run stats, activity timeline, blueprint success rates</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>Blueprints</strong> — Browse, create, edit, run, and manage blueprint YAML files</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>Pipelines</strong> — Chain blueprints into multi-stage workflows</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>Run Log</strong> — History of all blueprint runs with status, duration, and details</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>Schedules</strong> — Cron-based recurring runs and one-time scheduled executions</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>Telemetry</strong> — JSONL activity logs per run for debugging and auditing</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>Settings</strong> — Execution limits, safety controls, telemetry retention</div>
            </div>
          </div>
          ...
          <div className="hud-panel" style={{ padding: '1rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              {[
                ...
                <div key={label} style={{ fontSize: '0.8rem', color: 'rgb(var(--muted))' }}>
                  {label}: <span style={{ fontFamily: 'monospace', color: 'rgb(var(--accent))' }}>{path}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {tab === 'blueprints' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', maxWidth: '640px' }}>
          ...
          <div className="hud-panel" style={{ padding: '1rem' }}>
            ...
            <div style={{ fontSize: '0.78rem', color: 'rgb(var(--muted))', lineHeight: '1.6' }}>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>version:</strong> Schema version (currently "0.4")</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>blueprint:</strong> name, slug, title, description, author, license, tags, homepage</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>project:</strong> type, name, dir, path, php_version, node, vars</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>env:</strong> key-value environment variables</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>inputs:</strong> name, label, type (string|number|boolean|array), required, default, enum, pattern</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>packages:</strong> npm, composer, pip, brew, go</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>steps:</strong> sections with tasks (name, cmd, call, if, retry, timeout, dir, env)</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>hooks:</strong> before_run, after_run, on_error</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>imports:</strong> path, alias, with</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>stubs:</strong> enabled, search_paths, strict_match</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>git:</strong> init, create_github_repo, visibility, remote, branch</div>
            </div>
          </div>
          ...
          <div className="hud-panel" style={{ padding: '1rem' }}>
            ...
            <div style={{ fontSize: '0.78rem', color: 'rgb(var(--muted))', lineHeight: '1.6' }}>
              <div style={{ marginBottom: '0.5rem', color: 'rgb(var(--text))' }}>
                  ...
                  {[
                    ...
                    <tr key={variable}>
                      <td style={{
                        ...
                        color: 'rgb(var(--accent))', whiteSpace: 'nowrap', width: '220px',
                      }}>{variable}</td>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.3rem 1.5rem' }}>
                {[
                  ...
                  <div key={fn} style={{ display: 'flex', gap: '0.5rem', alignItems: 'baseline' }}>
                    <span style={{ fontFamily: 'monospace', fontSize: '0.75rem', color: 'rgb(var(--accent))' }}>{fn}</span>
          ...
          <div className="hud-panel" style={{ padding: '1rem' }}>
            ...
            <div style={{ fontSize: '0.78rem', color: 'rgb(var(--muted))', lineHeight: '1.6' }}>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>cmd</strong> — Shell command to execute</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>call</strong> — Call another blueprint by path</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>if</strong> — Conditional expression (template syntax)</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>dir</strong> — Working directory for the command</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>env</strong> — Per-task environment variables</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>retry</strong> — Number of retry attempts on failure</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>retry_delay_seconds</strong> — Delay between retries</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>timeout_seconds</strong> — Maximum execution time</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>continue_on_error</strong> — Don&apos;t halt on failure</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>on_success / on_fail</strong> — Hooks triggered by outcome</div>
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'pipelines' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', maxWidth: '640px' }}>
          ...
          <div className="hud-panel" style={{ padding: '1rem' }}>
            ...
            <div style={{ fontSize: '0.78rem', color: 'rgb(var(--muted))', lineHeight: '1.6' }}>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>meta.name</strong> — Pipeline display name</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>stop_on_fail</strong> — Stop pipeline if a stage fails (default: true)</div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>stages[]</strong> — Ordered list of stages to execute:</div>
              <div style={{ paddingLeft: '1rem' }}>
                <div><strong style={{ color: 'rgb(var(--accent))' }}>name</strong> — Stage identifier (required)</div>
                <div><strong style={{ color: 'rgb(var(--accent))' }}>blueprint_path</strong> — Path to blueprint YAML (required)</div>
                <div><strong style={{ color: 'rgb(var(--accent))' }}>inputs</strong> — Key-value inputs passed to the blueprint</div>
              </div>
              <div><strong style={{ color: 'rgb(var(--accent))' }}>inputs</strong> — Global inputs inherited by all stages</div>
            </div>
          </div>
          ...
          <div className="hud-panel" style={{ padding: '1rem' }}>
            ...
            <pre style={{
              fontSize: '0.75rem', lineHeight: '1.5', color: 'rgb(var(--text))',
              background: 'rgb(var(--panel2))', padding: '0.75rem', borderRadius: '4px',
              border: '1px solid rgb(var(--border))', overflow: 'auto',
  ...
  - name: deploy
    blueprint_path: ./deploy.yaml`}
          ...
          <div className="hud-panel" style={{ padding: '1rem' }}>
            ...
            <div style={{ fontSize: '0.78rem', color: 'rgb(var(--muted))', lineHeight: '1.6' }}>
              <div>Stages execute <strong style={{ color: 'rgb(var(--text))' }}>sequentially</strong> — each stage waits for the previous one to complete.</div>
              <div style={{ marginTop: '0.3rem' }}>With <strong style={{ color: 'rgb(var(--accent))' }}>stop_on_fail: true</strong> (default), the pipeline halts when any stage fails.</div>
              <div style={{ marginTop: '0.3rem' }}>With <strong style={{ color: 'rgb(var(--accent))' }}>stop_on_fail: false</strong>, all stages run regardless of failures.</div>
              <div style={{ marginTop: '0.3rem' }}>Each stage creates a separate <strong style={{ color: 'rgb(var(--text))' }}>blueprint run</strong> visible in the Run Log.</div>
              <div style={{ marginTop: '0.3rem' }}>Default stage timeout: <strong style={{ color: 'rgb(var(--text))' }}>60 seconds</strong>. Blueprint-level timeouts override this.</div>
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'examples' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', maxWidth: '640px' }}>
          <div style={{ fontSize: '0.78rem', color: 'rgb(var(--muted))', lineHeight: '1.5' }}>
            Sample blueprints and pipelines for common use cases. <strong style={{ color: 'rgb(var(--text))' }}>View</strong> to
            see the full YAML, <strong style={{ color: 'rgb(var(--text))' }}>Copy</strong> to clipboard,
            or <strong style={{ color: 'rgb(var(--text))' }}>Add</strong> to save directly to your blueprints folder.
          </div>
          ...
          {filteredSamples.map(sample => (
            <div key={sample.filename} className="hud-panel" style={{ padding: '0.75rem 1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
                <span style={{
                  ...
                  color: sample.kind === 'pipeline' ? 'rgb(var(--accent))' : 'rgb(var(--ok))',
                }}>
                  {sample.kind}
                </span>
                <div style={{ fontWeight: 600, fontSize: '0.85rem', color: 'rgb(var(--text))', flex: 1 }}>
                  {sample.title}
                </div>
                <div style={{ display: 'flex', gap: '0.25rem' }}>
                  <button
              ...
              <div style={{ fontSize: '0.68rem', fontFamily: 'monospace', color: 'rgb(var(--accent))', marginTop: '0.2rem' }}>
                {sample.filename}
              </div>
            </div>
          ))}
        ...
        <div className="hud-modal-overlay" onClick={() => setViewingSample(null)}>
          <div className="hud-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '650px', width: '100%' }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <span style={{
                    ...
                    color: viewingSample.kind === 'pipeline' ? 'rgb(var(--accent))' : 'rgb(var(--ok))',
                  }}>
                    {viewingSample.kind}
                  </span>
                  <span style={{ fontWeight: 600, fontSize: '0.9rem' }}>{viewingSample.title}</span>
                </div>
                <div style={{ fontSize: '0.72rem', fontFamily: 'monospace', color: 'rgb(var(--accent))' }}>{viewingSample.filename}</div>
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '0.75rem' }}>
              <button className="hud-button-ghost" onClick={() => setViewingSample(null)}>Close</button>
            </div>
          </div>
        </div>
      )}
