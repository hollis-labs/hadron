...
import { openDirectoryDialog, listFilesInDir, validateBlueprintFile, enqueueRun, getPreference, setPreference, parseBlueprintInputs, deleteBlueprintFile, getBlueprintMetadata, getSettings, createDirectory, selectDirectoryDialog, moveBlueprintFile, copyBlueprintFile, archiveBlueprintFile } from '../api/client';
...
export function BlueprintsPage({ daemonStatus, workspaceId, onRunCreated, onOpenBlueprint, onNewBlueprint, onBatchRunComplete }: BlueprintsPageProps) {
  ...
  // Restore last-used folder from preferences on mount, fallback to default dir
  useEffect(() => {
    getPreference('lastBlueprintDir').then(async saved => {
      if (saved) {
        ...
        const defaultDir = await getPreference('defaultBlueprintDir');
        if (defaultDir) {
          setRootDir(defaultDir);
