// Package inmemory provides the public concurrency-safe, process-lifetime
// implementation of the workflow runtime storage contracts.
//
// Store preserves the same CAS, idempotency, defensive-copy, recovery-query,
// and append-only event semantics required of durable adapters. Its data exists
// only for the life of one Store value: it does not survive process restart,
// coordinate across processes, or provide a reopen contract. It is suitable
// for embedded/offline execution, contract qualification, examples, and tests;
// a host that promises crash recovery must supply durable storage instead.
package inmemory
...
// core lifecycle and storage semantics, but no scheduling policy or durable
// restart guarantee.
type Store struct {
	mu sync.RWMutex
...
// NewStore returns an empty process-lifetime StateStore.
func NewStore() *Store {
	return &Store{
