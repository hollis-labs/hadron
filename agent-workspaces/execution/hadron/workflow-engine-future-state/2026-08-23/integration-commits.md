...
| 67 | W06-T04 | `e423176` | `05a97b5` | Hadron | accepted after owner-scoped durable A2A task/run correlation, exact published-registry skill projection, intent-bound idempotency, append-only persistence, confirmation escalation, restart recovery, and fail-closed HTTP routing |
| 68 | W05-T05 | `bb72580` | `f0df790` | Hadron | accepted after locator-sensitive exact snapshot identity, atomic source/compile capture, immutable run links, explicit unavailable fallback, bounded retention cleanup, and shared credential-safe locator projection |
...
| 79 | Dependency publication cleanup | `agentkit@cc43681`, `go-scheduler@51ebe8a` | `4484aa1` | Hadron + go-scheduler | published `go-scheduler` v0.1.1 at the exact durable fire/attempt commit, aligned `agentkit` to v0.5.0, and removed both local module replacements without changing compiled source |
| 80 | W07-T10 | `1d3033e` | `0cf8718` | Hadron | accepted with graph-visible dormant handlers, durable per-run saga ledgers, reverse-dependency best-effort unwind, child-owned ledgers, compensation-before-finally admission, typed evidence, bounded operator controls, replay attestations, SQLite/in-memory parity, and conformance fixtures |
| 81 | W07-T10-H1 | root integration review | `46a6afb` | Hadron | cleared the complete release-lint gate without suppressions, made SQL row cleanup explicit, completed enum handling, removed dead helpers, and retained all focused, race, full-repository, frontend, and E2E behavior |
